 ...
 * 简洁版本，保留核心演示目的。
 *
 * 编译: gcc -Wall -Wextra performance_demo_concise.c -o
 * performance_demo_concise -pthread -std=c11 运行: ./performance_demo_concise
 *
 * 注意: 为了看到明显的效果，最好在多核 CPU 系统上运行。
 *       CPU_WORK_FACTOR 和 SYNC_INCREMENTS 的值可能需要根据你的 CPU
 * 性能进行调整。
 */
...
AlignedCounter counter_array_for_false_sharing
    [NUM_THREADS];  // 用于伪共享/对齐演示的计数器数组
// ----------------------------
...
void *cpu_bound_task() {
    ...
    for (int i = 0; i < CPU_WORK_FACTOR; ++i) { local_sum += i; }
 ...
 *        此函数用于两个场景：
 *        1. 伪共享场景：当 `AlignedCounter`
 * 数组元素间距不足以避免伪共享时（本例中已通过 padding
 * 避免，但为了演示概念，可以想象 padding 被移除）。
 ...
 */
void run_and_time_tasks(void *(*task_func)(void *), const char *description,
                        void **task_args) {
    ...
    for (long i = 0; i < NUM_THREADS; ++i) {
        void *arg = (task_args == NULL)
                        ? NULL
                        : task_args[i];  // 根据是否有参数数组选择
        ret = pthread_create(&threads[i], NULL, task_func, arg);
        if (ret != 0)
            handle_pthread_error(ret, "pthread_create (%s) failed.",
                                 description);
    }
    for (int i = 0; i < NUM_THREADS; ++i) {
        ret = pthread_join(threads[i], NULL);
        if (ret != 0)
            handle_pthread_error(ret, "pthread_join (%s) failed.", description);
    }
...
int main() {
    ...
    // 初始化线程索引参数
    for (i = 0; i < NUM_THREADS; ++i) { thread_indices[i] = (void *)i; }

    printf("--- C 语言多线程性能考量演示 (简洁版) ---\n");
    printf(
        "线程数: %d, CPU密集型工作/线程: %d, 同步递增量/线程: %d, "
        "伪共享递增量/线程: %d\n\n",
        NUM_THREADS, CPU_WORK_FACTOR, SYNC_INCREMENTS,
        FALSE_SHARING_INCREMENTS);
    ...
    run_and_time_tasks(mutex_protected_counter_task, "   互斥量保护计数器",
                       NULL);
    printf("   总和: %lld (期望总和: %d)\n", global_sum_mutex,
           NUM_THREADS * SYNC_INCREMENTS);
    printf(
        "   预期: "
        "性能通常远低于无锁并行，甚至比单线程更慢，因为锁开销和串行化。\n\n");
    pthread_mutex_destroy(&sum_mutex);  // 销毁互斥量

    // --- 4. 多线程共享计数器 (原子操作保护) ---
    printf("4. 多线程共享计数器 (原子操作保护)\n");
    global_sum_atomic = 0;  // 重置
    run_and_time_tasks(atomic_protected_counter_task, "   原子操作保护计数器",
                       NULL);
    printf("   总和: %lld (期望总和: %d)\n", global_sum_atomic,
           NUM_THREADS * SYNC_INCREMENTS);
    ...
    // 所以，我们假设 false_sharing_counters
    // 数组在概念上代表了“可能发生伪共享”的紧凑数据， 而通过 AlignedCounter 的
    // padding 解决了伪共享。

    // 重置伪共享计数器数组
    for (i = 0; i < NUM_THREADS; ++i)
        counter_array_for_false_sharing[i].value = 0;
    printf("5. 伪共享演示 (概念层面解释，这里实际上已通过缓存行对齐避免)\n");
    run_and_time_tasks(counter_array_task, "   可能伪共享的任务",
                       (void **)thread_indices);
    long long total_counter_sum = 0;
    for (i = 0; i < NUM_THREADS; ++i)
        total_counter_sum += counter_array_for_false_sharing[i].value;
    printf("   总和: %lld (期望总和: %lld)\n", total_counter_sum,
           (long long)NUM_THREADS * FALSE_SHARING_INCREMENTS);
    printf(
        "   预期: "
        "即使每个线程修改不同索引，若无正确对齐，数据在同一缓存行会导致伪共享，"
        "性能下降。\n");
    printf(
        "         (本代码中已通过 padding "
        "避免伪共享，所以此处的性能应较好)\n\n");
    ...
    run_and_time_tasks(counter_array_task, "   缓存行对齐任务",
                       (void **)thread_indices);
    total_counter_sum = 0;
    for (i = 0; i < NUM_THREADS; ++i)
        total_counter_sum += counter_array_for_false_sharing[i].value;
