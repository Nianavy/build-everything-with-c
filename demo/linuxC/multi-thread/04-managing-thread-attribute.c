...
static void handle_pthread_error(const char *func_name, int err_code) {
    fprintf(stderr, "Error in %s: %d (%s)\n", func_name, err_code,
            strerror(err_code));  // 修正 strerror 的使用
    exit(EXIT_FAILURE);
}
...
void *thread_function() {
    ...
    ret = pthread_getaffinity_np(tid, sizeof(cpu_set_t),
                                 &cpuset);  // _GNU_SOURCE 和 <sched.h>
    if (ret != 0) {
...
int main() {
    ...
    if (ret != 0) { handle_pthread_error("pthread_attr_init", ret); }
    ...
    // PTHREAD_EXPLICIT_SCHED 表示线程的调度策略和优先级由 attr
    // 指定，而不是继承父线程的
    ...
    CPU_SET(0, &cpuset);  // 将 CPU 0 加入集合 (核心修正: CPU_SET 需要 <sched.h>
                          // 且 _GNU_SOURCE)
    ...
    if (ret != 0) { handle_pthread_error("pthread_join", ret); }

    // 8. 销毁线程属性对象，释放其占用的资源
    ret = pthread_attr_destroy(&attr);
    if (ret != 0) { handle_pthread_error("pthread_attr_destroy", ret); }
