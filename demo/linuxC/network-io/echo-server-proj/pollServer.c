...
void poll_(int listen_sock) {
    ...
    while (true) {
        ...
        for (int fd = 0; fd <= maxfd; ++fd) {
            if (fds[fd].fd < 0 || fds[fd].revents != POLLIN) continue;
            fds[fd].revents = 0;
            if (fd == listen_sock) {
                connection_t conn = accept_connection(listen_sock);
                if (conn.sock == -1) continue;
                fds[conn.sock].fd = conn.sock;
                fds[conn.sock].events = POLLIN;
                fds[conn.sock].revents = 0;

                if (conn.sock > maxfd) maxfd = conn.sock;
                continue;
            }
            else {
                args_t cbargs = { .fds = fds, .maxfd = &maxfd };
