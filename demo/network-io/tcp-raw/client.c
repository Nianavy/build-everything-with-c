...
void send_hello(const int fd) {
    ...
    *data = htonl(100);              // 载荷数据，这里发送数字 100，转换为网络字节序
    ...
    printf("Sent PROTO_HELLO message (type %d, len %d, data %d), %zd bytes.\n", 
           (int)ntohl(hdr->type), (int)ntohs(hdr->len), (int)ntohl(*data), bytes_sent);
}
...
void receive_and_print_response(const int fd) {
    ...
    if (bytes_received < (ssize_t)sizeof(proto_hdr_t)) {
        fprintf(stderr, "receive_response: Incomplete header received (%zd bytes).\n", bytes_received);
        return;
    }

    // 解析协议头
    proto_type_e type = (proto_type_e)ntohl(hdr->type); // 消息类型从网络字节序转换
    ...
    if (len > 0) {
        if (len > (unsigned short)(sizeof(buf) - sizeof(proto_hdr_t))) {
            fprintf(stderr, "receive_response: Payload too large for buffer (len %hu).\n", len);
            return; // 载荷太大无法放入缓冲区
        }
        ...
        if (bytes_received < len) {
            fprintf(stderr, "receive_response: Incomplete payload received (%zd of %hu bytes).\n", bytes_received, len);
            return;
        }
