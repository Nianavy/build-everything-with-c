...
/*
    mutex -> mutual exclusion (access)  // 互斥量：用于互斥访问共享资源（同一时间只有一个线程能访问）
    semaphore -> multiple access (signaling) // 信号量：可以允许多个访问，或用于事件通知
*/
...
void *signal_event() { // 这是线程 t2 将执行的函数
    ...
    sem_post(&sem); // **V 操作：将信号量 sem 的值加 1。如果 sem=0，现在就变成 1。如果此时有线程在 sem_wait 处阻塞，它将被唤醒。**
...
void *wait_for_event() { // 这是线程 t1 将执行的函数
    ...
    sem_wait(&sem); // **P 操作：尝试将信号量 sem 的值减 1。**
                    // **由于在 main 函数中 sem 初始化为 0，当 t1 第一次调用 sem_wait 时，它发现 sem 的值不是 >0 的，因此 t1 线程会被立即阻塞在这里，直到 sem 的值被其他线程通过 sem_post 增加。**
    printf("Event has been triggered!\n"); // 当 t1 被唤醒并成功将 sem 减 1 后，它会继续执行这里
    return NULL; // 线程函数必须返回 NULL 或一个指针
}
