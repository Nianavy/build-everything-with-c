 ...
 *
 * 编译死锁演示: gcc -Wall -Wextra -DDEMO_DEADLOCK=1 deadlock_unified_demo.c -o deadlock_unified_demo -pthread
 * 运行: ./deadlock_unified_demo  (程序会死锁，需要 Ctrl+C 终止)
 *
 * 编译死锁避免演示: gcc -Wall -Wextra deadlock_unified_demo.c -o deadlock_unified_demo -pthread
 * 运行: ./deadlock_unified_demo (程序将正常完成)
 */
...
void *thread_func(void *arg) {
    ...
    if (thread_id == 1) { // 线程 A 模仿：mutex1 -> mutex2
        ...
        if (ret != 0) handle_pthread_error(ret, "Thread %d: pthread_mutex_lock (mutex1) failed.", thread_id);
        printf("Thread %d: Locked mutex1. Doing some work, then attempting to lock mutex2...\n", thread_id);
        sleep(1); 

        printf("Thread %d: Attempting to lock mutex2...\n", thread_id);
        ret = pthread_mutex_lock(&mutex2);
        if (ret != 0) handle_pthread_error(ret, "Thread %d: pthread_mutex_lock (mutex2) failed.", thread_id);
        ...
        if (ret != 0) handle_pthread_error(ret, "Thread %d: pthread_mutex_unlock (mutex2) failed.", thread_id);
        printf("Thread %d: Releasing mutex1...\n", thread_id);
        ret = pthread_mutex_unlock(&mutex1);
        if (ret != 0) handle_pthread_error(ret, "Thread %d: pthread_mutex_unlock (mutex1) failed.", thread_id);
    } else { // 线程 B 模仿：mutex2 -> mutex1
        printf("Thread %d: Attempting to lock mutex2...\n", thread_id);
        ret = pthread_mutex_lock(&mutex2);
        if (ret != 0) handle_pthread_error(ret, "Thread %d: pthread_mutex_lock (mutex2) failed.", thread_id);
        printf("Thread %d: Locked mutex2. Doing some work, then attempting to lock mutex1...\n", thread_id);
        sleep(1);
...
#else // 死锁避免模式 (DEMO_DEADLOCK == 0)：所有线程都按照约定顺序 (mutex1 -> mutex2) 获取锁
    printf("Thread %d: Attempting to lock mutex1...\n", thread_id);
...
int main() {
    ...
    printf("This program is expected to deadlock. Press Ctrl+C to terminate.\n");
#else
    printf("--- DEADLOCK AVOIDANCE DEMONSTRATION (Ordered Resource Allocation) ---\n");
    printf("This program is expected to complete normally.\n");
#endif

    // 创建两个线程
    ret_A = pthread_create(&tid_A, NULL, thread_func, &id_A);
    if (ret_A != 0) handle_pthread_error(ret_A, "pthread_create (Thread 1) failed.");
    
    ret_B = pthread_create(&tid_B, NULL, thread_func, &id_B);
    if (ret_B != 0) handle_pthread_error(ret_B, "pthread_create (Thread 2) failed.");

    // 等待线程完成
    // 在死锁模式下，pthread_join 会一直等待，直到被外部终止
    ret_A = pthread_join(tid_A, NULL);
    if (ret_A != 0) handle_pthread_error(ret_A, "pthread_join (Thread 1) failed.");
    
    ret_B = pthread_join(tid_B, NULL);
    if (ret_B != 0) handle_pthread_error(ret_B, "pthread_join (Thread 2) failed.");

#if DEMO_DEADLOCK == 1
    printf("--- PROGRAM UNEXPECTEDLY FINISHED (NO DEADLOCK), SOMETHING IS WRONG ---\n");
#else
    ...
    if (ret_A != 0) handle_pthread_error(ret_A, "pthread_mutex_destroy (mutex1) failed.");
    ret_B = pthread_mutex_destroy(&mutex2);
    if (ret_B != 0) handle_pthread_error(ret_B, "pthread_mutex_destroy (mutex2) failed.");
