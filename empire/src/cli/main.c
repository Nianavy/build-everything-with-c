...
int send_hello(int fd) {
    ...
    dbproto_hello_req *hello_req = (dbproto_hello_req *)(buf + sizeof(dbproto_hdr_t)); // 请求体指针
    ...
    // 发送完整的 Hello 请求消息
    if (send_full(fd, buf, sizeof(dbproto_hdr_t) + sizeof(dbproto_hello_req)) == STATUS_ERROR) {
    ...
    if (hdr->type == MSG_ERROR) {
        ...
        if (hdr->len != sizeof(dbproto_hello_resp)) {
            fprintf(stderr, "Error: Hello response length mismatch. Expected %zu, got %u.\n", sizeof(dbproto_hello_resp), hdr->len);
            return STATUS_ERROR;
        }
        // 接收 Hello 响应体
        dbproto_hello_resp *hello_resp = (dbproto_hello_resp *)(buf + sizeof(dbproto_hdr_t));
        if (read_full(fd, hello_resp, sizeof(dbproto_hello_resp)) == STATUS_ERROR) {
...
int send_add_employee_req(int fd, const char *addstring) {
    ...
    dbproto_employee_add_req_t *add_req = (dbproto_employee_add_req_t *)(buf + sizeof(dbproto_hdr_t));

    // 检查添加字符串长度是否超出协议限制
    size_t addstring_len = strlen(addstring);
    if (addstring_len >= MAX_EMPLOYEE_ADD_DATA) {
        fprintf(stderr, "Error: Employee add string too long (%zu bytes), max is %d.\n", addstring_len, MAX_EMPLOYEE_ADD_DATA - 1);
        return STATUS_ERROR;
    }
    ...
    // 发送完整的添加员工请求消息
    if (send_full(fd, buf, sizeof(dbproto_hdr_t) + sizeof(dbproto_employee_add_req_t)) == STATUS_ERROR) {
    ...
    if (hdr->type == MSG_ERROR) {
        ...
        // 接收响应体
        dbproto_employee_add_resp_t *add_resp = (dbproto_employee_add_resp_t *)(buf + sizeof(dbproto_hdr_t));
        if (read_full(fd, add_resp, sizeof(dbproto_employee_add_resp_t)) == STATUS_ERROR) {
        ...
        fprintf(stderr, "Unexpected message type for add employee response: %d\n", hdr->type);
        return STATUS_ERROR;
    }
}
...
int send_list_employee_req(int fd) {
    ...
    if (hdr->type == MSG_ERROR) {
        ...
        if (hdr->len != sizeof(dbproto_employee_list_resp_t)) {
            fprintf(stderr, "Error: List employee response length mismatch. Expected %zu, got %u.\n", sizeof(dbproto_employee_list_resp_t), hdr->len);
            return STATUS_ERROR;
        }
        // 接收响应体（包含员工总数）
        dbproto_employee_list_resp_t *list_resp = (dbproto_employee_list_resp_t *)(buf + sizeof(dbproto_hdr_t));
        if (read_full(fd, list_resp, sizeof(dbproto_employee_list_resp_t)) == STATUS_ERROR) {
        ...
        if (list_resp->count == 0) {
            ...
            for (uint16_t i = 0; i < list_resp->count; ++i) {
                struct employee_t employee_data; // 声明一个临时 employee_t 结构体来接收
                if (read_full(fd, &employee_data, sizeof(struct employee_t)) == STATUS_ERROR) {
        ...
        fprintf(stderr, "Unexpected message type for list employee response: %d\n", hdr->type);
        return STATUS_ERROR;
    }
}
...
int send_remove_employee_req(int fd) {
    ...
    if (hdr->type == MSG_ERROR) {
        ...
        if (hdr->len != sizeof(dbproto_employee_del_resp_t)) {
            fprintf(stderr, "Error: Remove employee response length mismatch.\n");
            return STATUS_ERROR;
        }
        // 接收响应体
        dbproto_employee_del_resp_t *del_resp = (dbproto_employee_del_resp_t *)(buf + sizeof(dbproto_hdr_t));
        if (read_full(fd, del_resp, sizeof(dbproto_employee_del_resp_t)) == STATUS_ERROR) {
        ...
        if (del_resp->status == STATUS_SUCCESS) {
            ...
            fprintf(stderr, "Failed to remove employee on server (status: %d).\n", del_resp->status);
            return STATUS_ERROR;
        }
    } else {
        fprintf(stderr, "Unexpected message type for remove employee response: %d\n", hdr->type);
        return STATUS_ERROR;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    int c;
    // 解析命令行参数：支持 -p (端口), -h (主机), -a (添加), -l (列出), -r (删除)
    ...
    if (action_count > 1) {
        fprintf(stderr, "Error: Client can only perform one action at a time (-a, -l, or -r).\n");
        return STATUS_ERROR;
    }
    if (action_count == 0) {
